...
use axum::{
    ...
    http::StatusCode,
    extract::{Path as AxumPath, Query, ws::{WebSocketUpgrade, WebSocket, Message}},
    Json, Router,
};
...
async fn add_torrent(Json(payload): Json<AddPayload>) -> impl IntoResponse {
    match execute_ipc(IpcMessage::Add {
        torrent_path: payload.torrent_path,
        download_dir: payload.download_dir,
    }) {
        Ok(IpcReply::Success { message }) => (StatusCode::OK, Json(serde_json::json!({ "status": "success", "message": message }))).into_response(),
...
async fn pause_torrent(AxumPath(info_hash): AxumPath<String>) -> impl IntoResponse {
...
async fn resume_torrent(AxumPath(info_hash): AxumPath<String>) -> impl IntoResponse {
...
async fn delete_torrent(AxumPath(info_hash): AxumPath<String>, Query(params): Query<DeleteParams>) -> impl IntoResponse {
