...
impl Selector {
    ...
    pub fn local_piece_suggestions(&mut self, remote_peer: &Peer, number_of_suggestions: usize) -> Vec<u32> {
        ...
        let max_suggestions = std::cmp::min(remote_peer.number_of_missing_pieces as usize, number_of_suggestions);
        ...
        while suggestions.len() < max_suggestions {
            if !remote_peer.is_piece_on_remote_peer(current_piece)
                && tc_guard.as_ref().map_or(false, |tc| tc.is_piece_local(current_piece))
    ...
    pub fn get_list_of_peers(&self, tc: &TorrentContext, piece_number: u32, max_peers: usize) -> Vec<String> {
        let mut peers: Vec<(i64, String)> = tc
            ...
            .filter_map(|(ip, peer)| {
                ...
                if peer.connected && peer.peer_choking.wait_one(0) && peer.is_piece_on_remote_peer(piece_number) {
        ...
        peers.into_iter().take(max_peers).map(|(_, ip)| ip).collect()
    }
}
