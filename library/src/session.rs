    ...
    pub fn connect_and_download_peer(
        ...
        let handle = thread::spawn(move || {
...

            {
                ...
                // Send our bitfield so the remote peer knows which pieces we have.
                let bitfield = context.lock().unwrap().bitfield.clone();
                let _ = peer_guard.send_bitfield(bitfield);
                // Unchoke unconditionally — we are willing to serve data.
                let _ = peer_guard.send_message(PeerMessage::Unchoke);
                ...
                match tracker.announce_once() {
                    Ok(response) => {
                        for peer_details in response.peer_list {
                            ...
                            let info_hash = ctx2.lock().unwrap().info_hash.clone();
                            let local_peer_id = peer_id::get();

                            if let Some(ref mgr) = mgr2 {
                            ...
                            thread::spawn(move || {
                                let address =
                                    format!("{}:{}", peer_details.ip, peer_details.port);
                                let stream = match address
                                    .parse::<std::net::SocketAddr>()
                                    .ok()
                                    .and_then(|addr| {
...

                                {
                                    ...
                                    pg.set_torrent_context(ctx2.clone());
                                    if pg
                                        .handshake(&info_hash, local_peer_id.as_bytes())
                                        .is_err()
                                    {
                                        if let Some(mgr) = &mgr2 {
                                            mgr.add_to_dead_peer_list(&peer_details.ip);
                                    ...
                                    let bitfield =
                                        ctx2.lock().unwrap().bitfield.clone();
                                    let _ = pg.send_bitfield(bitfield);
                                    let _ = pg.send_message(PeerMessage::Unchoke);
                                    pg.am_choking = false;
                                ...
                                loop {
                                    ...
                                    if pg.peer_choking.wait_one(0)
                                        && ctx.status == TorrentStatus::Downloading
                                    {
                                        let to_send = 10usize
                                            .saturating_sub(pg.outstanding_requests_count);
                                        for _ in 0..to_send {
                                            match ctx.next_block_request_for_peer(&pg) {
                                                Some((pn, begin, length)) => {
                                                    if pg
                                                        .send_request(pn, begin, length)
                                                        .is_err()
                                                    {
                                                        if let Some(mgr) = &mgr2 {
                                                            mgr.add_to_dead_peer_list(
